...
(
	...
	0,
	TEXT("Draws debug info about footsteps/n  0: off/n  1: on/n"),
	ECVF_Cheat
);
...
void USPFootstepsComponent::HandleFootstep(EFoot Foot)
{
	...
	if (Character)
	{
		...
		if (USkeletalMeshComponent* SkeletalMesh = Character->GetMesh())
		{
			...
			GetWorld()->LineTraceSingleByChannel(HitResult, StartTracePoint, EndTracePoint, ECollisionChannel::ECC_WorldStatic, QueryParams);
